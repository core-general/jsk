...
public class JskCsQueueBackImplTest extends JskCsAbstractQueueTest<JskCsBothType, JskCsQueueBackImpl<?, Integer, ?>> {
    @Override
    ...
    @Override
    protected JskCsBothType getForwardDirection() {
        return JskCsBothType.FORWARD;
    }
    ...
    public void iteratorBackTest() {
        ...
        assertEquals("", format(queue1.getBackItems()));
        assertEquals("",
                format(Cc.list(() -> (Iterator<JskCsItem<?, Integer, JskCsBothType, ?>>) (Object) queue1.iteratorBack())));
        queue1.poll();
        queue1.poll();
        queue1.poll();
        queue1.poll();
    ...
    @NotNull
    private String format(List<? extends JskCsItem<?, Integer, JskCsBothType, ?>> items) {
