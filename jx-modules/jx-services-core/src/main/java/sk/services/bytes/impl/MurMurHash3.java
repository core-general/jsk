package sk.services.bytes.impl;
...
public class MurMurHash3 {
    ...
    /**
     * Compute the MurMurHash3 hash as described in the original source code.
     *
...
//			int k1 = (data[i4] & 0xff) + ((data[i4 + 1] & 0xff) << 8) + ((data[i4 + 2] & 0xff) << 16) + ((data[i4 + 3] & 0xff)
//			<< 24);
    ...
    /**
     * Compute the MurMurHash3 hash (128-bit version) as described in the original source code.
     *
     * @param data   the data that needs to be hashed
     * @param length the length of the data that needs to be hashed
     * @param seed   the seed to use to compute the hash
