...
public class RemoteCollector {
        ...
        if (!aggregationApplication) {
            for (final URL url : urlsForCollect) {
                try {
                    ...
                    final List<Serializable> serialized = new MelRemoteCollectorPatch()//JSK patch here
                            .getRemoteOrLoadLocal(url, createRemoteCall(url), collector, application);
    ...
    public Map<JavaInformations, List<CounterRequestContext>> collectCurrentRequests()
        ...
        final Map<JavaInformations, List<CounterRequestContext>> result =
                new LinkedHashMap<JavaInformations, List<CounterRequestContext>>();
        for (final URL url : getURLs()) {
