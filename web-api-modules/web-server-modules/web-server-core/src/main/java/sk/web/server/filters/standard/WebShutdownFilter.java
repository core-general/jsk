...
public class WebShutdownFilter implements WebServerFilter {
    public static final int PRIORITY = WebExceptionFilter.PRIORITY + PRIORITY_STEP;

    @Inject IWebExcept webExcept;

    @Override
    public <API> WebFilterOutput invoke(WebServerFilterContext<API> requestContext) {
        return !requestContext.getRequestContext().isShouldStop()
                ? requestContext.getNextInChain().invokeNext()
                : webExcept.throwByCode(503, "must_retry");
    }
